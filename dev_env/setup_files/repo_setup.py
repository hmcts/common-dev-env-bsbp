...
def run_script_files(file_path: str, list_of_files: list, service_name: str, key_vault: str):

    logger.info('test runs')
    ...
    if len(list_of_files) >= 2 and 'setup-env.sh' in list_of_files and 'create-env-file.sh' in list_of_files:
        ...
        call_command('sudo %s/setup-env.sh %s %s aat' % (file_path, key_vault, service_name))
    # else: 
