...
def create_repo_if_required(service_name: str, file_path: str, git_url: str):
    if does_path_exist(file_path):
        ...
        if not does_path_exist(file_path):
            logging.info(
                'Error whilst cloning. Please review and also make sure you have your SSH key set up on GitHub')
            quit()
...
def copy_environment_vars(dir_path: str, environment_vars: dict, service_name: str):
    ...
    if not does_file_exist(file_to_path, file_name):
        logger.info('Creating env var substitutions file %s for %s and saving in project directory /bin' % (
...
def run_script_files(file_path: str, list_of_files: list, service_name: str, key_vault: str, service_type: str,
                     all_docker_per_service: bool, prompt_to_create_env: bool, chart_location: str):
    if len(list_of_files) >= 2 and 'setup-env.sh' in list_of_files and 'create-env-file.sh' in list_of_files:
        ...
        call_command('sudo %s/setup-env.sh %s %s aat %s %s %s %s' % (
