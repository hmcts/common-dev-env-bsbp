...
def check_version_requirements():
    logger.debug('Checking bash version is supported through command: "bash --version"')
    if is_bash_version_not_supported():
        ...
        logger.debug('Bash version is supported, proceeding to next step')

    logger.debug('Checking python version is supported')
    if is_python_version_not_supported():
        ...
        logger.debug('Python version is supported, proceeding to next step')

    logger.debug('Checking yq version. This needs to basically be installed')
    if is_yq_version_not_supported():
        ...
        logger.debug('Yq is installed/supported, proceeding to next step')
